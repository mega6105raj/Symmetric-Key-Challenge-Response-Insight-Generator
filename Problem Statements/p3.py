...
# Convert boolean-like text to numeric
...
# (c) Timestamp difference (session-to-session)
df = df.sort_values(by='timestamp')
df['time_diff'] = df['timestamp'].diff().fillna(0)

# ---------------------------
# 4. Encode Protocol Type
# ---------------------------
le = LabelEncoder()
...
# Features that may influence success
features = [
    'protocol_type_enc',
    'response_time',
    'nonce_entropy',
...
# 8. Train Model
# ---------------------------
# Gradient Boosting works well for mixed features
